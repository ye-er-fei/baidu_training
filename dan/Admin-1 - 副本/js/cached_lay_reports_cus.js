        ...
        if(!$this.hasClass('on')){
            ...
            if(ul.length > 0){
                ul.slideDown('fast',function(){
                    $this.addClass('on');
                    $('.nav-item').removeClass('current-item'); 
                    $this.addClass('current-item');

                    $('.nav-item').find('.icon').removeClass('item-1').addClass('item-2');
                    //icon.removeClass('item-2');
        	        icon.removeClass('item-2').addClass('item-1'); 
	...
	return {
		...
		init : function(target,options){
			...
			var defaults = {
				...
				panelTempl : '<div class="filterpanel" style="display:none;"><input type="text" class="input" placeholder="'+I18n.t('components.search.name')+'"/><ul class="filterlist"></ul><div class="load" style="margin:10px auto;text-align:center;display:block;"><img src="images/ajax-loader.gif"/></div><div class="submitpanel"><a href="#" class="submit">'+I18n.t('components.buttons.confirm')+'</a></div></div>'
			};
			var settings = $.extend({},defaults,options);
...
//                },
       ...
       renderCompareChart: function(options){
           ...
           try{
               ...
               this.element.after('<div class="loadingChart"><img src="images/ajax-loader.gif" /></div>');
               $.ajax({
                   type: 'get',
                   url: u,
...
;(function($,U){
    ...
    Template.prototype.temps = {
        plugin:{
            chartLoading: '<div class="load-chart"><p><img src="images/ajax-loader.gif"></p></div>',
...
;(function($) {
  $.widget('umeng.multiSelector', {
    ...
    templatePicker: '<div class="multi-selector-options"> \
      <div class="loading-tip" ><img src="images/ajax-loader.gif"></div> \
      </div> \
